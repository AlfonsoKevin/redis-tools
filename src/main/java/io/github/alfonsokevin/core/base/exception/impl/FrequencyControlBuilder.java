...
public class FrequencyControlBuilder {
     ...
     */
    private static final ConcurrentHashMap<Class<? extends AbstractRedisToolsException>,
            Constructor<? extends AbstractRedisToolsException>> CACHE_MAP = new ConcurrentHashMap<>();
     ...
     * @return
     */
    public static AbstractRedisToolsException build(
            Class<? extends AbstractRedisToolsException> exceptionClass, String message) {
        // 如果存在则返回，不存在则构建
        try {
            Constructor<? extends AbstractRedisToolsException> constructor =
                    CACHE_MAP.computeIfAbsent(exceptionClass, c -> {
                        try {
